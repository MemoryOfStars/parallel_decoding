...
void* decode_thread(void* params) 
{
	...
	while (av_read_frame(pFormatCtx, &packet) == 0) 
	{
		...
		if (packet.stream_index == videoindex && packet.size != 0) 
		{
			...
			{
				//send frame
				ret = av_interleaved_write_frame(decode_group_list[group_id].ic, &packet);
				if (ret == 0)
				{
					cout << "#" << flush;
				}
...
int main(int argc, char* argv[])
{
	...
	for(int group_id = 0;group_id < TOTAL_GROUP; group_id++)
	{
		ret = avformat_alloc_output_context2(&decode_group_list[group_id].ic, 0, "rtsp", decode_group_list[group_id].OutUrl);
		if (ret != 0)
		{
			char buf[1024] = { 0 };
		...
		//b 添加视频流 
		decode_group_list[group_id].vs = avformat_new_stream(decode_group_list[group_id].ic, NULL);
		if (!decode_group_list[group_id].vs)
		{
			//throw exception("avformat_new_stream failed");
			cout<<" avformat_new_stream failed"<<endl;
		}
		decode_group_list[group_id].vs->codecpar->codec_tag = 0;
		//从编码器复制参数
		avcodec_parameters_from_context(decode_group_list[group_id].vs->codecpar, decode_group_list[group_id].items[0].pFormatCtx->streams[0]->codec);
		av_dump_format(decode_group_list[group_id].ic, 0, decode_group_list[group_id].OutUrl, 1);


		///打开rtmp 的网络输出IO
		ret = avio_open(&decode_group_list[group_id].ic->pb, decode_group_list[group_id].OutUrl, AVIO_FLAG_WRITE);
		if (ret != 0)
		{
			cout<<ret<<endl;
			char buf[1024] = { 0 };
		...
		//写入封装头
		ret = avformat_write_header(decode_group_list[group_id].ic, NULL);
		if (ret != 0)
		{
			cout<<ret<<endl;
			char buf[1024] = { 0 };
	...
	{
		decode_id[i] = i;
		pthread_create(&decode_tid[i], 0, decode_thread, (void*)&decode_id[i]);
	}
